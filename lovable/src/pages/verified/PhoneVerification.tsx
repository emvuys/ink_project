...
const VerifiedPhoneVerification = () => {
  const [value, setValue] = useState("");
  const navigate = useNavigate();

  const handleComplete = (digits: string) => {
    if (digits.length === 4) {
      // Navigate to delivery record page after entering digits
      setTimeout(() => {
        navigate("/verified/delivery-record");
      }, 500);
    }
  };
...
export default VerifiedPhoneVerification;